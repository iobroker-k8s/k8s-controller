...
async function initializeController(): Promise<void> {
  ...
  if (ioPackage.notifications) {
    try {
      ...
      logger.error(
        `${hostLogPrefix} Could not add notifications config of this host: ${e.message}`
      );
    }
  }*/
...
async function processMessage(msg: ioBroker.SendableMessage): Promise<null | void> {
    ...
    case "getLocationOnDisk":
      if (msg.callback && msg.from) {
        sendTo(
          msg.from,
          msg.command,
          { path: controllerDir, platform: os.platform() },
          msg.callback
        );
      } else {
        logger.error(
    ...
    case "getLogFile":
      if (msg.callback && msg.from && msg.message) {
        ...
        if (
          ...
          config.log.transport[msg.message.transport]
        ) {
      ...
      break;*/
        ...
        for (const _id of Object.keys(procs)) {
          if (procs[_id].process) {
            outputCount++;
            states!.setState(`${_id}.checkLogging`, {
              val: true,
              ack: false,
              from: hostObjectPrefix,
            });
    ...
    case "upload": {
      if (msg.message) {
        ...
        logger.error(
          `${hostLogPrefix} No adapter name is specified for upload command from  ${msg.from}`
        );
      }
      break;
    }*/
      ...
      if (!msg.message.id) {
        if (msg.callback && msg.from) {
          sendTo(
            msg.from,
            msg.command,
            { error: "Adapter to rebuild not provided." },
            msg.callback
          );
        }
    ...
    case "writeBaseSettings": {
      ...
      if (!config.system) {
        return sendResponseTo({
          receivedMsg: msg,
          payload: { error: 'Cannot find "system" in data' },
        });
      }
      if (!config.objects) {
        return sendResponseTo({
          receivedMsg: msg,
          payload: { error: 'Cannot find "objects" in data' },
        });
      }
      if (!config.states) {
        return sendResponseTo({
          receivedMsg: msg,
          payload: { error: 'Cannot find "states" in data' },
        });
      }
      if (!config.log) {
        return sendResponseTo({
          receivedMsg: msg,
          payload: { error: 'Cannot find "log" in data' },
        });
      }
      ...
      try {
        ...
        return sendResponseTo({
          receivedMsg: msg,
          payload: { error: `Cannot write file ${configFile}` },
        });
      }
    ...
    case "upgradeOsPackages": {
      ...
      try {
        ...
        logger.warn(
          `${hostLogPrefix} Could not check for new OS updates after upgrade: ${e.message}`
        );
      }
    ...
    case "sendToSentry": {
      ...
      if (!sentryObj) {
        logger.debug(
          `${hostLogPrefix} Do not send message "${message}" to Sentry, because it is disabled`
        );
        return;
      }
...
async function getInstances(): Promise<void> {
  ...
  if (instances.length === 0) {
    ...
    const _ipArr = tools.findIPs();
    logger.info(
      `${hostLogPrefix} ${instances.length} instance${instances.length === 1 ? "" : "s"} found`
    );
    let count = 0;
    ...
    for (const instance of instances) {
      ...
      logger.debug(
        `${hostLogPrefix} check instance "${instance._id}" for host "${instance.common.host}"`
      );
      console.log(
        `${hostLogPrefix} check instance "${instance._id}" for host "${instance.common.host}"`
      );
...
function checkAndAddInstance(instance: ioBroker.InstanceObject, ipArr: string[]): boolean {
  ...
  if (!instance.common.host) {
    ...
    objects!.setObject(instance._id, instance, (err) =>
      err
        ? logger.error(
            `${hostLogPrefix} Cannot update hostname for ${instance._id}: ${err.message}`
          )
  ...
  for (const [id, proc] of Object.entries(procs)) {
    if (
      ...
      if (!id.startsWith(`${SYSTEM_ADAPTER_PREFIX}admin`)) {
        ...
        if (proc.process) {
          logger.info(
            `${hostLogPrefix} instance "${id}" was not started, because already running.`
          );
          continue;
        }
...
function checkVersion(
  ...
  if (!isFound) {
    ...
    for (const inst of filteredInst) {
      if (
        version &&
        !semver.satisfies(instances[inst].common.version, version, { includePrerelease: true })
      ) {
        throw new Error(
  ...
  if (!storeTimer) {
    storeTimer = setTimeout(() => {
      ...
      try {
        ...
        logger.error(
          `${hostLogPrefix} Please consider running the installation fixer when on Linux.`
        );
      }
    }, 1_000);
  }
...
async function startScheduledInstance(callback?: () => void): Promise<void> {
  ...
  if (!proc.lastStart || Date.now() - proc.lastStart >= 2_000) {
    ...
    if (!proc.process) {
      ...
      await states!.setState(`${instance._id}.sigKill`, {
        val: 0,
        ack: false,
        from: hostObjectPrefix,
      });
...
async function startInstance(id: ioBroker.ObjectIDs.Instance, wakeUp = false): Promise<void> {
  ...
  if (
    Array.isArray(instance.common.nodeProcessParams) &&
    instance.common.nodeProcessParams.length
  ) {
  ...
  try {
    ...
    if (ioPack.common.version !== instance.common.version) {
      logger.warn(
        `${hostLogPrefix} Detected missing upload of adapter "${name}" - starting upload now.`
      );
  ...
  if (instance.common.onlyWWW) {
    logger.debug(
      `${hostLogPrefix} startInstance ${name}.${instanceNo} only WWW files. Nothing to start`
    );
    return;
  }
  ...
  if (
    ...
    availableMemMB <
      (typeof config.system.memLimitWarn === "number" ? config.system.memLimitWarn : 100)
  ) {
    if (
      availableMemMB <
      (typeof config.system.memLimitError === "number" ? config.system.memLimitError : 50)
    ) {
      logger.error(
  ...
  switch (mode) {
            ...
            if (proc?.needsRebuild) {
              ...
              if (
                ...
                if (code === EXIT_CODES.UNCAUGHT_EXCEPTION) {
                  ...
                  logger.debug(
                    `${hostLogPrefix} Reset crash count of ${id}, because non-crash exit`
                  );
                  proc.crashCount = 0;
                  if (proc.crashResetTimer) {
                    logger.debug(
        ...
        const handleAdapterProcessStart = (): void => {
          ...
          if (!proc.process) {
            ...
            try {
              ...
              logger.error(
                `${hostLogPrefix} instance ${instance._id} could not be started: ${err}`
              );
            }
          }
    ...
    case "schedule": {
      ...
      try {
        ...
        logger.error(
          `${hostLogPrefix} Cannot schedule start of instance ${instance._id}: ${e.message}`
        );
        break;
      }
      ...
      logger.info(
        `${hostLogPrefix} instance scheduled ${instance._id} ${instance.common.schedule}`
      );
      ...
      if (instance.common.allowInit) {
        try {
          ...
          logger.info(
            `${hostLogPrefix} instance ${instance._id} could not be started: ${e.message}`
          );
        }
        if (proc.process) {
          storePids();
  ...
  switch (instance.common.mode) {
    case "daemon":
      ...
      if (force && !proc.startedAsCompactGroup) {
        logger.info(
          `${hostLogPrefix} stopInstance forced ${instance._id} killing pid ${proc.process.pid}`
        );
        proc.stopping = true;
        try {
        ...
        const timeoutDuration =
          supportStopInstanceVal === true ? 1_000 : supportStopInstanceVal || 1_000;
        return new Promise((resolve) => {
